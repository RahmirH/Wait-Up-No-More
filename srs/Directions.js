<View style={{display: this.state.directions}}>
<View style={styles.contentContainer}>
<Image 
source={{uri: 'data:image/png;base64,'}}
style={{height: 103, width: 155, marginTop: 25, margin: 10,}}/>
<Text style={styles.title}>
What else can I do?
</Text>
<Text style={styles.text4}>
Now you have the option of speaking directly to someone who works at this location and sees the area everyday:
</Text>
<Text style={styles.text5}>
  What should I ask?
</Text>
<Text style={styles.text4}>
-Make a reservation
-Pictures of the inside 
-When is it the most/least busy 
-Anythig else you may want to know
</Text>
<TouchableHighlight style = {styles.button1} onPress={this.handleTextsPagePress}>
<Text style={styles.buttonText}>
Speak to A Manager
</Text>
</TouchableHighlight>
<TouchableHighlight style = {styles.button1} onPress={this.handleDirectionsPagePress}>
<Text style={styles.buttonText}>
Revisit Statistics 
</Text>
</TouchableHighlight>
</View>
</View>
<View style={{display: this.state.texts}}>
<View style={styles.contentContainer2}>
<TextInput style={styles.textBar3} onChangeText={(userText)=> 
this.setState({userText})} value={this.state.userText}/>
<TextInput style={styles.textBar3} onChangeText={(workerText)=> 
this.setState({workerText})} value={this.state.workerText}/>
<TextInput style={styles.textBar3} onChangeText={(userText)=> 
this.setState({userText})} value={this.state.userText}/>
<TextInput style={styles.textBar3} onChangeText={(workerText)=> 
this.setState({workerText})} value={this.state.workerText}/>
<TextInput style={styles.textBar3} onChangeText={(userText)=> 
this.setState({userText})} value={this.state.userText}/>
<TextInput style={styles.textBar3} onChangeText={(workerText)=> 
this.setState({workerText})} value={this.state.workerText}/>
</View>
</View>
</View>
);
    }
}
