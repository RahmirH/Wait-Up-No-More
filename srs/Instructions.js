<View style={{display: this.state.instructions}}>
<View style={styles.contentContainer}>
<Image 
source={{uri: 'data:image/png;base64,'}}
style={{height: 103, width: 155, marginTop: 25, margin: 10,}}/>
<Text style={styles.title}>
What is this App?
</Text>
<Text style={styles.text4}>
Instructions:
</Text>
<Text style={styles.text5}>
  This app will allow you to make your plans without having to worry about having to make late plan changes because you need a faster table or don't want to many people around. Now, tell us about some of your favorite places to visit!
</Text>
<TouchableHighlight style = {styles.button1} onPress={this.handleFavoritesPagePress}>
<Text style={styles.buttonText}>
Continue
</Text>
</TouchableHighlight>
<TouchableHighlight style = {styles.button1} onPress={this.handleVerifyPagePress}>
<Text style={styles.buttonText}>
See Last Page 
</Text>
</TouchableHighlight>
</View>
</View>
